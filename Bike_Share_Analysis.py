...
# 
# **Answer**: 1.How many active users per day?
# 2.How much distance each single user travelled per day?
# 3.what is the starting and destination address of the users?
# 4.what is the day by day growth of the users?
# 
...
def duration_in_mins(datum, city):
    ...
    if(city =='NYC'):
        fmt=('%H:%M:%S')
        duration1=dt.datetime.strptime(datum['stoptime'].split()[-1],fmt)-dt.datetime.strptime(datum['starttime'].split()[-1],fmt)
        duration=(duration1.seconds)/60
    elif(city=='Chicago'):
        duration1=datum['tripduration']
...
def time_of_trip(datum, city):
    ...
    if(city=='NYC' or city=='Chicago'):
        my_date=datetime.datetime.strptime(''.join(datum['starttime'].split()[0].split('/')), "%m%d%Y").date()
        #my_date=datum['starttime'].split()[0].split('/')
        day_of_week=calendar.day_name[my_date.weekday()]
        month=int((datum['starttime'].split()[0].split('/'))[0])
        hour=int(datum['starttime'].split()[-1].split(':')[0])
        
    else:
        month=int(datum['Start date'].split()[0].split('/')[0])
        hour=int(datum['Start date'].split()[-1].split(':')[0])
        my_date=datetime.datetime.strptime(''.join(datum['Start date'].split()[0].split('/')), "%m%d%Y").date()
        day_of_week=calendar.day_name[my_date.weekday()]
...
# 
# **Answer**: NYC has the max number of trips, the number is 276798
...
# 
# **Answer**: itook Chicago and customers takes longer rides on average than subscribers
...
with open('./data/Chicago-Divvy-2016.csv') as f:
    ...
    for row in tripreader:
        if(row['usertype']=='Subscriber'):
            c+=1
            duration1=dt.strptime(row['stoptime'].split()[-1],fmt)-dt.strptime(row['starttime'].split()[-1],fmt)
            duration=(duration1.seconds)/60
            duration2+=duration
        else:
            d+=1
            ...
            duration=(duration1.seconds)/60
            duration3+=duration
    duration4=(duration2+duration3)/(c+d)
    duration2/=c
    duration3/=d
    print('%.2f'%duration2,'%.2f'%duration3,'%.2f'%duration4)
...
# 
# **Answer**:  for Washington Customer Trip Durations, the peak is in 15-20 mins bin.
#  the histogram for registered users is right skewed.
#  and as their is no other type of users in washingtone the other type is flat it has nothing to polat graph
...
plt.hist(substriber_data,bins,histtype='bar',rwidth=0.8)
...
plt.title('Distribution of Washington Rigestered Trip Durations')
plt.xlabel('Duration (m)')
plt.legend()
plt.show()
...
# 
# **Answer**:   The month of july has highest ridership.
#  The month of june has highest subscriber ridership.
#  The month of july has highest customer ridership.
#  The ridership is high during weekdays. The ridership for both subscribers and customers is high during weekdays.
